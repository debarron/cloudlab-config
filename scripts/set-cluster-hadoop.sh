...
ssh -o "StrictHostKeyChecking no" -i "$private_key" "$username@$master_node" "~/$hadoop_script $install_id $data_dir && hostname | cut -d'.' -f1 > ~/masters" &> /dev/null

# 2 Configure masternode
echo -e ">> CONFIGURING MASTER NODE 😎\n"
ssh_command="
sudo cp -r /proj/nosql-json-PG0/hadoop_$install_id $data_dir/hadoop
